    ...
    for
      ...
      node <- div(
        ...
        p(
          "I am",
          span(
            styleAttr := "color:mediumseablue;font-weight:bold", "@nvshinde"
          ),
...
private object CatShooter {
  ...
  def shootLaser(
    ...
    fs2.Stream
      .range(1, Size)
      .evalMap { row =>
        ref.update(_.copy(laserRow = Some(row)))
      }
      .metered(20.millis)
      .compile 
      .drain >> 
      ref.modify { s =>
        if s.cat == s.mouse then
          ...
          (s.copy(laserRow = None), false)
      }.flatMap {
        case true =>
          randCol(rng).flatMap(newCol =>
            ref.update(_.copy(mouse = newCol))
            )
        case false => IO.unit 
      }
