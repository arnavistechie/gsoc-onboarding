  ...
  object Quote:
    given Decoder[Quote] = Decoder.instance { c =>
      c.downField("content").as[String].flatMap { content =>
        c.downField("author").as[String].map { author => Quote(content, author) }
      }
    }
    ...
    Resource.eval(fetchQuote.flatMap(q => state.set(State(q, false, true)))) *>
      div(
        styleAttr <-- state.map { s =>
          ...
          s"padding: 24px; background: $bg; color: $textColor; border-radius: 4px; max-width: 480px; border: 1px solid ${
              if s.darkMode then "#4a463d" else "#c9c2b4"
        ...
        div(
          ...
          p(
            styleAttr <-- state.map(s =>
              s"text-align: right; color: ${
                  if s.darkMode then "#9a9286" else "#6b6560"
